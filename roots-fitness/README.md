...
- セミパーソナルのペアレッスン料金
- **セミパーソナルの単発(都度)利用の料金**: 2026年8月、チケット制(4回/16回/24回の回数券)が終了し月額制に一本化。単発利用は引き続き可能とのことでニュース掲載・プラン欄に項目を追加したが、料金が未確定のため `data/plans.json` の `semi-single` は `price: null`(表示は「料金未定」)のまま。確定次第、金額を差し替える
