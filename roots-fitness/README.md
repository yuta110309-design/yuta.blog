...
- **実績セクションの数値**: Google口コミ(45件・★5.0)はGoogleマップの「Roots FITNESS LOUNGE KARUIZAWA」のプロフィールで確認できた実数(軽井沢店単独)を反映済み。ただし ① **累計セッション実績はまだ未確認**(プレースホルダー `---` のまま)、② **代官山店に別のGoogleビジネスプロフィールがあるか**、あれば両店舗合算にするか店舗別表示のままにするか方針を確認したい、③ 集計時点(例: 2026年◯月現在)の注記も追加したい
