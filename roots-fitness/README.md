...
- **セミパーソナルの料金プラン**: チケット制終了後の構成として、週1回コース(月4回・24,000円)、週2回コース(月8回・40,000円)、週3回コース(月12回・48,000円)、通い放題(33,000円)、単発都度(12,000円)、ペアレッスン(14,000円/お一人様7,000円)を反映済み。
  - **要確認**: 週2回コース(40,000円)・週3回コース(48,000円)が、無制限の通い放題(33,000円)より高額になっている。月8回以上通う方は通い放題を選んだ方が安くなる価格構成のため、意図した設計(あえて通い放題へ誘導する)か、単価の見直しが必要かを確認したい
...
- **ラグジュアリーカード(LC)会員様向け優待プランへの参画**: 加盟が決定した旨を `news-005`(2026-09-05付、予約投稿)としてお知らせに仕込み済み。優待内容(割引率・対象プラン・利用開始日・申込方法など)が確定次第、本文とリンク先を追記する
  - **掲載決定時、LCのロゴ・画像を追加する**(現時点ではLC側の正式な掲載可否が未確定のため、LCの公式ロゴ・商標は未使用。先方の許諾が取れた正式なロゴデータ・使用ガイドラインが揃った段階で、ニュース記事や提携訴求箇所に反映する)
