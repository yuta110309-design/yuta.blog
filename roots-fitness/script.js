(function () {
  ...
  /* ------------------------------------------------------------------ */
  /* トップページの「お客様の声」ティザー(店舗ごとに分けて数件だけ表示し、       */
  /* 「もっと見る」タップでその店舗の残りを表示する)。                        */
  ...
  if (voiceTeaser) {
    ...
    function buildVoiceTeaserCardHtml(review, isExtra) {
      return (
    ...
    function buildVoiceTeaserGroupHtml(store, groupIndex) {
      var reviews = store.reviews || [];
      if (!reviews.length) return "";

      var cardsHtml = reviews
        .map(function (review, i) {
          return buildVoiceTeaserCardHtml(review, i >= voiceTeaserInitial);
        })
        .join("");

      var toggleHtml =
        reviews.length > voiceTeaserInitial
          ? '<button type="button" class="btn btn-line btn-small" data-voice-teaser-toggle="' + groupIndex + '">もっと見る</button>'
          : "";

      return (
        '<div class="results-voice-group">' +
          '<h3 class="results-voice-group-title">' + store.storeName + "</h3>" +
          '<div class="results-voice-cards" data-voice-teaser-cards="' + groupIndex + '">' + cardsHtml + "</div>" +
          (toggleHtml ? '<div class="results-voice-actions">' + toggleHtml + "</div>" : "") +
        "</div>"
      );
    }
    ...
    fetch(voiceTeaser.getAttribute("data-voice-teaser-source"))
      ...
      .then(function (data) {
        var stores = (data.stores || []).filter(function (store) {
        ...
        voiceTeaser.innerHTML = stores.map(buildVoiceTeaserGroupHtml).join("");

        stores.forEach(function (store, groupIndex) {
          var toggle = document.querySelector('[data-voice-teaser-toggle="' + groupIndex + '"]');
          var cardsEl = document.querySelector('[data-voice-teaser-cards="' + groupIndex + '"]');
          if (!toggle || !cardsEl) return;
          toggle.addEventListener("click", function () {
            var expanded = cardsEl.classList.toggle("is-expanded");
